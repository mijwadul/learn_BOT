  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
      <div className="relative w-full max-w-xl bg-[#0d111a] border border-red-500/30 rounded-2xl shadow-2xl shadow-red-950/60 flex flex-col max-h-[92vh] overflow-hidden">
        ...
        {/* Modal Footer with Safety Guard */}
        <div className="p-4 border-t border-white/10 bg-[#0a0d14] flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="w-full sm:w-auto">
            <span className="text-[11px] text-red-300/80 font-semibold block">Ketik <span className="font-mono text-red-400 font-bold">RESET</span> untuk konfirmasi:</span>
            <input
              type="text"
              value={confirmText}
              onChange={(e) => setConfirmText(e.target.value)}
              placeholder="RESET"
              className="mt-1 px-3 py-1.5 rounded-lg bg-black/60 border border-red-500/30 text-red-300 text-xs font-mono uppercase tracking-widest outline-none focus:border-red-400 w-full sm:w-36 text-center"
            />
          </div>
          <div className="flex items-center gap-2.5 w-full sm:w-auto justify-end">
            ...
            <button
              ...
              disabled={isResetting || selectedItems.length === 0 || confirmText.trim().toUpperCase() !== "RESET"}
              className="px-5 py-2 rounded-xl text-xs font-bold text-white bg-red-600 hover:bg-red-500 shadow-lg shadow-red-900/50 flex items-center gap-2 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
            >
              {isResetting ? (
